...
def _build_event_metrics_internal(
    *,
    signal_risk_score: float,
    feature_confidence_used: float,
    feature_confidence_raw: float,
    feature_confidence_calibrated: float,
    ...
    gate_soft_entry_vote_count: int,
    periodicity_component_acf: float,
    periodicity_component_spectrum: float,
    periodicity_component_envelope: float,
    periodicity_component_fft: float,
    periodicity_component_freq_agreement: float,
    periodicity_freq_welch_hz: float,
    periodicity_freq_zerocross_hz: float,
    periodicity_freq_fft_hz: float,
    signal_tail_amplitude: float,
    signal_tail_decay_proxy: float,
    feature_rms_decay_local: float,
    feature_rms_decay_local_r2: float,
    feature_rms_decay_local_window_count: int,
    gate_rms_decay_local_on_ok: int,
    gate_rms_decay_local_off_hint: int,
    feature_rms_decay_event: float,
    feature_rms_decay_event_r2: float,
    feature_rms_decay_event_window_count: int,
    feature_rms_decay_event_window_sec: float,
    signal_reason: str,
    fsm_transition_reason: str,
    state_evidence: float,
    feature_long_ratio_on: float,
    feature_long_ratio_off: float,
    feature_long_ratio_off_recent: float,
    feature_long_off_recent_count: int,
    gate_long_off_vote_sum: int,
    gate_long_off_vote_count: int,
    periodicity_acf_peak: float,
    periodicity_acf_period_sec: float,
    periodicity_acf_lag_steps: int,
    periodicity_acf_window_count: int,
    feature_long_zmax: float,
    feature_long_window_count: int,
    feature_baseline_window_count: int,
) -> dict[str, object]:
    """Build semantically named metrics before legacy event export flattening."""

    return {
        "signal_risk_score": float(signal_risk_score),
        "feature_confidence_used": float(feature_confidence_used),
        "feature_confidence_raw": float(feature_confidence_raw),
        "feature_confidence_calibrated": float(feature_confidence_calibrated),
        ...
        "gate_soft_entry_vote_count": int(gate_soft_entry_vote_count),
        "periodicity_component_acf": float(periodicity_component_acf),
        "periodicity_component_spectrum": float(periodicity_component_spectrum),
        "periodicity_component_envelope": float(periodicity_component_envelope),
        "periodicity_component_fft": float(periodicity_component_fft),
        "periodicity_component_freq_agreement": float(periodicity_component_freq_agreement),
        "periodicity_freq_welch_hz": float(periodicity_freq_welch_hz),
        "periodicity_freq_zerocross_hz": float(periodicity_freq_zerocross_hz),
        "periodicity_freq_fft_hz": float(periodicity_freq_fft_hz),
        "signal_tail_amplitude": float(signal_tail_amplitude),
        "signal_tail_decay_proxy": float(signal_tail_decay_proxy),
        "feature_rms_decay_local": float(feature_rms_decay_local),
        "feature_rms_decay_local_r2": float(feature_rms_decay_local_r2),
        "feature_rms_decay_local_window_count": int(feature_rms_decay_local_window_count),
        "gate_rms_decay_local_on_ok": int(gate_rms_decay_local_on_ok),
        "gate_rms_decay_local_off_hint": int(gate_rms_decay_local_off_hint),
        "feature_rms_decay_event": float(feature_rms_decay_event),
        "feature_rms_decay_event_r2": float(feature_rms_decay_event_r2),
        "feature_rms_decay_event_window_count": int(feature_rms_decay_event_window_count),
        "feature_rms_decay_event_window_sec": float(feature_rms_decay_event_window_sec),
        "signal_reason": str(signal_reason),
        "fsm_transition_reason": str(fsm_transition_reason),
        "state_evidence": float(state_evidence),
        "feature_long_ratio_on": float(feature_long_ratio_on),
        "feature_long_ratio_off": float(feature_long_ratio_off),
        "feature_long_ratio_off_recent": float(feature_long_ratio_off_recent),
        "feature_long_off_recent_count": int(feature_long_off_recent_count),
        "gate_long_off_vote_sum": int(gate_long_off_vote_sum),
        "gate_long_off_vote_count": int(gate_long_off_vote_count),
        "periodicity_acf_peak": float(periodicity_acf_peak),
        "periodicity_acf_period_sec": float(periodicity_acf_period_sec),
        "periodicity_acf_lag_steps": int(periodicity_acf_lag_steps),
        "periodicity_acf_window_count": int(periodicity_acf_window_count),
        "feature_long_zmax": float(feature_long_zmax),
        "feature_long_window_count": int(feature_long_window_count),
        "feature_baseline_window_count": int(feature_baseline_window_count),
    }
...
def _build_event_metrics_kwargs(
    ...
    """Flatten internal semantic metrics to stable external event payload keys."""

    return {
        "score": float(internal["signal_risk_score"]),
        "confidence": float(internal["feature_confidence_used"]),
        "confidence_raw": float(internal["feature_confidence_raw"]),
        "confidence_cal": float(internal["feature_confidence_calibrated"]),
        "cal_on_conf_thr": float(internal["gate_calibration_confidence_threshold"]),
        "on_support": float(internal["feature_support_score"]),
        "on_support_ema": float(internal["feature_support_ema"]),
        "on_soft_votes_sum": int(internal["gate_soft_entry_vote_sum"]),
        "on_soft_votes_n": int(internal["gate_soft_entry_vote_count"]),
        "c_acf": float(internal["periodicity_component_acf"]),
        "c_spec": float(internal["periodicity_component_spectrum"]),
        "c_env": float(internal["periodicity_component_envelope"]),
        "c_fft": float(internal["periodicity_component_fft"]),
        "c_freq_agree": float(internal["periodicity_component_freq_agreement"]),
        "f_welch": float(internal["periodicity_freq_welch_hz"]),
        "f_zc": float(internal["periodicity_freq_zerocross_hz"]),
        "f_fft": float(internal["periodicity_freq_fft_hz"]),
        "A_tail": float(internal["signal_tail_amplitude"]),
        "D_tail": float(internal["signal_tail_decay_proxy"]),
        "rms_decay": float(internal["feature_rms_decay_local"]),
        "rms_decay_r2": float(internal["feature_rms_decay_local_r2"]),
        "rms_decay_n": int(internal["feature_rms_decay_local_window_count"]),
        "rms_decay_on_ok": int(internal["gate_rms_decay_local_on_ok"]),
        "rms_decay_off_hint": int(internal["gate_rms_decay_local_off_hint"]),
        "rms_decay_event": float(internal["feature_rms_decay_event"]),
        "rms_decay_event_r2": float(internal["feature_rms_decay_event_r2"]),
        "rms_decay_event_n": int(internal["feature_rms_decay_event_window_count"]),
        "rms_decay_event_win_sec": float(internal["feature_rms_decay_event_window_sec"]),
        "reason": str(internal["signal_reason"]),
        "transition_reason": str(internal["fsm_transition_reason"]),
        "evidence": float(internal["state_evidence"]),
        "long_ratio_on": float(internal["feature_long_ratio_on"]),
        "long_ratio_off": float(internal["feature_long_ratio_off"]),
        "long_ratio_off_recent": float(internal["feature_long_ratio_off_recent"]),
        "long_off_n_recent": int(internal["feature_long_off_recent_count"]),
        "long_off_votes_sum": int(internal["gate_long_off_vote_sum"]),
        "long_off_votes_n": int(internal["gate_long_off_vote_count"]),
        "acf_peak": float(internal["periodicity_acf_peak"]),
        "acf_period_sec": float(internal["periodicity_acf_period_sec"]),
        "acf_lag_steps": int(internal["periodicity_acf_lag_steps"]),
        "acf_n": int(internal["periodicity_acf_window_count"]),
        "long_zmax": float(internal["feature_long_zmax"]),
        "long_n": int(internal["feature_long_window_count"]),
        "baseline_n": int(internal["feature_baseline_window_count"]),
    }
...
def _build_tick_metrics_kwargs(
    *,
    ...
    metrics_internal = _build_event_metrics_internal(
        signal_risk_score=float(tick_signal.score),
        feature_confidence_used=float(tick_quality.confidence),
        feature_confidence_raw=float(tick_quality.confidence_raw),
        feature_confidence_calibrated=float(tick_quality.confidence_cal),
        gate_calibration_confidence_threshold=float(tick_quality.gate_calibration_confidence_threshold),
        feature_support_score=float(tick_quality.feature_support_score),
        feature_support_ema=float(tick_quality.feature_support_ema),
        gate_soft_entry_vote_sum=int(tick_vote.gate_soft_entry_vote_sum),
        gate_soft_entry_vote_count=int(len(st_votes.on_soft_votes)),
        periodicity_component_acf=float(tick_quality.c_acf),
        periodicity_component_spectrum=float(tick_quality.c_spec),
        periodicity_component_envelope=float(tick_quality.c_env),
        periodicity_component_fft=float(tick_quality.c_fft),
        periodicity_component_freq_agreement=float(tick_quality.c_freq_agree),
        periodicity_freq_welch_hz=float(tick_quality.f_welch),
        periodicity_freq_zerocross_hz=float(tick_quality.f_zc),
        periodicity_freq_fft_hz=float(tick_quality.f_fft),
        signal_tail_amplitude=float(tick_signal.A_tail),
        signal_tail_decay_proxy=float(tick_signal.D_tail),
        feature_rms_decay_local=float(tick_quality.rms_decay),
        feature_rms_decay_local_r2=float(tick_quality.rms_decay_r2),
        feature_rms_decay_local_window_count=int(tick_quality.rms_decay_n),
        gate_rms_decay_local_on_ok=int(tick_quality.rms_decay_on_ok),
        gate_rms_decay_local_off_hint=int(tick_quality.rms_decay_off_hint),
        feature_rms_decay_event=float(tick_quality.rms_decay_event),
        feature_rms_decay_event_r2=float(tick_quality.rms_decay_event_r2),
        feature_rms_decay_event_window_count=int(tick_quality.rms_decay_event_n),
        feature_rms_decay_event_window_sec=float(tick_quality.rms_decay_event_win_sec),
        signal_reason=str(tick_signal.reason),
        fsm_transition_reason=str(transition_reason),
        state_evidence=float(st_signal.evidence),
        feature_long_ratio_on=float(tick_signal.long_ratio_on),
        feature_long_ratio_off=float(tick_signal.long_ratio_off),
        feature_long_ratio_off_recent=float(tick_signal.long_ratio_off_recent),
        feature_long_off_recent_count=int(tick_signal.long_off_n_recent),
        gate_long_off_vote_sum=int(st_votes.long_off_votes.sum),
        gate_long_off_vote_count=int(len(st_votes.long_off_votes)),
        periodicity_acf_peak=float(tick_quality.acf_peak),
        periodicity_acf_period_sec=float(tick_quality.acf_period_sec),
        periodicity_acf_lag_steps=int(tick_quality.acf_lag_steps),
        periodicity_acf_window_count=int(tick_quality.acf_n),
        feature_long_zmax=float(tick_signal.long_zmax),
        feature_long_window_count=int(tick_signal.long_n),
        feature_baseline_window_count=int(len(st_cache.long_baseline_hist)),
    )
    return _build_event_metrics_kwargs(metrics_internal)
...
def _build_cached_metrics_kwargs(
    *,
    ...
    st_cache = st.cache
    metrics_internal = _build_event_metrics_internal(
        signal_risk_score=float("nan"),
        feature_confidence_used=float(q.confidence),
        feature_confidence_raw=float(q.confidence_raw),
        feature_confidence_calibrated=float(q.confidence_cal),
        gate_calibration_confidence_threshold=float("nan"),
        feature_support_score=float("nan"),
        feature_support_ema=float(st_signal.on_support_ema),
        gate_soft_entry_vote_sum=int(st_votes.on_soft_votes.sum),
        gate_soft_entry_vote_count=int(len(st_votes.on_soft_votes)),
        periodicity_component_acf=float(q.c_acf),
        periodicity_component_spectrum=float(q.c_spec),
        periodicity_component_envelope=float(q.c_env),
        periodicity_component_fft=float(q.c_fft),
        periodicity_component_freq_agreement=float(q.c_freq_agree),
        periodicity_freq_welch_hz=float(q.f_welch),
        periodicity_freq_zerocross_hz=float(q.f_zc),
        periodicity_freq_fft_hz=float(q.f_fft),
        signal_tail_amplitude=float("nan"),
        signal_tail_decay_proxy=float("nan"),
        feature_rms_decay_local=float(q.rms_decay_local),
        feature_rms_decay_local_r2=float(q.rms_decay_local_r2),
        feature_rms_decay_local_window_count=int(q.rms_decay_local_n),
        gate_rms_decay_local_on_ok=int(q.rms_decay_local_on_ok),
        gate_rms_decay_local_off_hint=int(q.rms_decay_local_off_hint),
        feature_rms_decay_event=float(q.rms_decay_event),
        feature_rms_decay_event_r2=float(q.rms_decay_event_r2),
        feature_rms_decay_event_window_count=int(q.rms_decay_event_n),
        feature_rms_decay_event_window_sec=float(q.rms_decay_event_win_sec),
        signal_reason=str(reason),
        fsm_transition_reason=str(transition_reason),
        state_evidence=float(st_signal.evidence),
        feature_long_ratio_on=float("nan"),
        feature_long_ratio_off=float("nan"),
        feature_long_ratio_off_recent=float("nan"),
        feature_long_off_recent_count=0,
        gate_long_off_vote_sum=int(st_votes.long_off_votes.sum),
        gate_long_off_vote_count=int(len(st_votes.long_off_votes)),
        periodicity_acf_peak=float(q.acf_peak),
        periodicity_acf_period_sec=float(q.acf_period_sec),
        periodicity_acf_lag_steps=int(q.acf_lag_steps),
        periodicity_acf_window_count=int(q.acf_n),
        feature_long_zmax=float("nan"),
        feature_long_window_count=int(len(st_cache.long_score_hist)),
        feature_baseline_window_count=int(len(st_cache.long_baseline_hist)),
    )
    return _build_event_metrics_kwargs(metrics_internal)
...
def _run_streaming_alert_demo_one_channel_cfg_impl(
    ...
    *,
    ...
    mp_runtime = _IntervalMPPostRuntime(
        ...
        mp_update_cadence_sec=float(mp.mp_update_cadence_sec),
        mp_adaptive_low_band_max_hz=float(mp.mp_adaptive_low_band_max_hz),
        mp_adaptive_high_band_min_hz=float(mp.mp_adaptive_high_band_min_hz),
        mp_freq_window_high_sec=float(mp.mp_freq_window_high_sec),
        mp_freq_window_mid_sec=float(mp.mp_freq_window_mid_sec),
        mp_freq_window_low_sec=float(mp.mp_freq_window_low_sec),
        mp_decay_window_high_sec=float(mp.mp_decay_window_high_sec),
        mp_decay_window_mid_sec=float(mp.mp_decay_window_mid_sec),
        mp_decay_window_low_sec=float(mp.mp_decay_window_low_sec),
        mp_decay_rms_win_sec=float(mp.mp_decay_rms_win_sec),
        mp_decay_step_sec=float(mp.mp_decay_step_sec),
        mp_decay_min_windows=int(mp.mp_decay_min_windows),
        mp_decay_min_r2=float(mp.mp_decay_min_r2),
        mp_decay_freq_jump_rel_max=float(mp.mp_decay_freq_jump_rel_max),
        ...
        mp_runtime_mode=str(mp_runtime_mode_eff),
    )
    ...
    for upd_idx, batch in enumerate(batch_iter, start=1):
        ...
        for key in touched_keys:
            ...
            phase_now, transition_reason = step_fsm(
                ...
                gate_calibration_active=bool(tick_quality.gate_calibration_active),
                feature_support_ema=float(tick_quality.feature_support_ema),
                gate_calibration_support_hold_min=float(pq.cal_on_support_hold_min),
                ...
                gate_soft_entry_confirmed=bool(tick_vote.gate_soft_entry_confirmed),
                gate_calibration_support_confirm_min=float(pq.cal_on_support_confirm_min),
            ...
            if print_tick:
                ...
                gate_onset_acceleration_ok = bool(gate_flags.accel_ok)
                gate_on_confidence_ok = bool(gate_flags.on_conf_ok)
                gate_on_support_ok = bool(gate_flags.on_support_ok)
                gate_long_ready = bool(gate_flags.long_ready)
                gate_on_long_ok = bool(gate_flags.on_long_gate_ok)
                ...
                status(
                    ...
                    f"G[accel={int(gate_onset_acceleration_ok)},conf={int(gate_on_confidence_ok)},support={int(gate_on_support_ok)},"
                    f"long_gate={int(gate_on_long_ok)},long_ready={int(gate_long_ready)},re_on={int(gate_re_on_active)},"
                    f"re_on_short={int(gate_re_on_short_ok)},re_on_accel={int(gate_re_on_accel_ok)},"
    ...
    for key, st in states.items():
        ...
        if not st_signal.on_event_emitted and np.isfinite(start_t):
            ...
            on_ev = {
                ...
                **_build_risk_event_metrics_from_kwargs(delayed_metrics),
            }
            events.append(on_ev)
        ...
        ev = {
            ...
            **_build_risk_event_metrics_from_kwargs(open_metrics),
    ...
    if interval_final_by_id and interval_samples_by_interval_id:
        ...
        def _pick_modal_freq_for_interval(interval_id: int) -> float:
            ...
            return float(
                mp_rec.get("mp_primary_mode_freq_hz", mp_rec.get("mp_dominant_freq_hz", np.nan))
            )
